...
module.exports = class LocaleLoader extends Loader {
   ...
   */
  downloadAndInitializeLocales (dirPath = '../locales') {
    return new Promise(async (resolve, reject) => {
      if (this.client.apis.crowdin) {
        console.log('Downloading locales from Crowdin', 'Localization')
        try {
          await this.client.apis.crowdin.downloadToPath(dirPath)
          console.log('[32mLocales downloaded', 'Localization')
        } catch (e) {
          console.log('[31mCouldn\'t download locales - An error ocurred.', 'Localization')
          console.logError(e)
        }
      } else {
        console.log('[31mCouldn\'t download locales - API wrapper didn\'t load.', 'Localization')
      }
      ...
      try {
        i18next.use(translationBackend).init({
          ...
          console.log('[32mi18next initialized', 'Localization')
        })
      } catch (e) {
        console.logError(e)
      }
